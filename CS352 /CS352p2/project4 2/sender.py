...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    while win_left_edge < INIT_SEQNO + content_len:
        ...
        ackMessage = False
    while not ackMessage:
        cango = select.select([cs],[],[], RTO)
        if cango[0]:
            ackMessage = True
            datafrom, receiver_addr = cs.recvfrom(100)
            ack_msg = Msg.deserialize(datafrom)
            print("{}".format(str(ack_msg)))
        else:
            newLeftEdge = transmit_one()    
            win_left_edge = newLeftEdge
